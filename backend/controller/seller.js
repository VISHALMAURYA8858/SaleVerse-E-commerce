...
router.post("/signup-seller", upload.single("file"), async (req, res, next) => {
    try {
        ...
        try {
            await sendMail({
                email: seller.email,
                subject: "Activate your SaleVerse Seller's Account",
                message: `Hello, please click on the link ${activationUrl} to verify your shop ${seller.name}. With regards from SaleVerse.`
            });
            res.status(201).json({
                success: true,
                message: `Please check your Email ${seller.email} to activate your SaleVerse Seller's account`,
            });
        }
        catch (error) {
