...
router.post("/create-user", upload.single("file"), async (req, res, next) => {
    try {
        ...
        try {
            await sendMail({
                email: user.email,
                subject: "Activate your SaleVerse Account",
                message: `Hello ${user.name}, please click on the link ${activationUrl} to verify your account. With regards from SaleVerse.`
            });
            res.status(201).json({
                success: true,
                message: `Please check your Email ${user.email} to activate your SaleVerse account`,
            });
        }
        catch (error) {
